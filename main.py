...
from utils import quote_url, stamp2time, time2stamp, time_comparison, get_heighest_video_quality, build_headers, print_info
...
_headers = build_headers(settings['cookie'])

request_count = 0    #请求次数计数
down_count = 0      #下载图片数计数

@retry(stop=stop_after_attempt(10), wait=wait_fixed(2))
def _get_other_info(_user_info):
    global request_count
    from utils import get_other_info
    result = get_other_info(_user_info, _headers, proxies)
    request_count += 1
    return result
...
def main(_user_info: object):
    ...
    if not _get_other_info(_user_info):
        return False
